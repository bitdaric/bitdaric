...
#if defined(NDEBUG)
# error "BitDaric cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "BitDaric Signed Message:\n";
...
CAmount GetMinRelayFee(const CTransaction& tx, unsigned int nBytes, bool fAllowFree)
{
    ...
    nMinFee += GetBitDaricDustFee(tx.vout, ::minRelayTxFee);
...
static bool ReadBlockOrHeader(T& block, const CDiskBlockPos& pos)
{
    ...
    // Check the header
    // BitDaric: We don't necessarily have block height, so we depend on using the base parameters
...
bool IsInitialBlockDownload()
{
    ...
    if (lockIBDState)
        return false;
	// Maximum tip age in seconds to consider node in initial block download (24 * 60 * 60)
	// a best header mined before 24 hours ago means the initial block is being downloading
    bool state = (chainActive.Height() < pindexBestHeader->nHeight - 24 * 6 ||
            pindexBestHeader->GetBlockTime() < GetTime() - 14 * 24 * 60 * 60);
    if (!state)
        lockIBDState = true;
    return state;
}
...
bool CheckInputs(const CTransaction& tx, CValidationState &state, const CCoinsViewCache &inputs, bool fScriptChecks, unsigned int flags, bool cacheStore, std::vector<CScriptCheck> *pvChecks)
{
    if (!tx.IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                // BitDaric: Switch maturity at depth 145,000
...
void ThreadScriptCheck() {
    RenameThread("bitdaric-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    //if (fEnforceBIP30) {
    // BitDaric: BIP30 has been active since inception
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
    ...
    // BitDaric: BIP16 has been active since inception
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase())
        {
            ...
            // BitDaric: BIP16 has been enabled since inception
            nSigOps += GetP2SHSigOpCount(tx, view);
            if (nSigOps > MAX_BLOCK_SIGOPS)
                return state.DoS(100, error("ConnectBlock(): too many sigops"),
                                 REJECT_INVALID, "bad-blk-sigops");
    ...
    CAmount blockReward = nFees + GetBitDaricBlockSubsidy(pindex->nHeight, consensus, hashPrevBlock);
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    // BitDaric: Version 2 enforcement was never used, reject from v3 softfork
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    // BitDaric: Block v2 was never enforced, so we trigger this against v3
